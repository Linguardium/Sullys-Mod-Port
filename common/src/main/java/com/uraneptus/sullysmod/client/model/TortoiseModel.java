...
public class TortoiseModel extends EntityModel<TortoiseRenderState> {
    ...
    @Override
    public void setupAnim(TortoiseRenderState entityRenderState) {
        super.setupAnim(entityRenderState);
        this.workstation_saddle.visible = entityRenderState.workstationRenderState.workstationType != SMWorkstationTypes.EMPTY_WORKSTATION_TYPE.get();
        this.head.xRot = entityRenderState.xRot * ((float)Math.PI / 180F);
        this.head.yRot = entityRenderState.yRot * ((float)Math.PI / 180F);

        if (entityRenderState.hideTimer == 0) {
            this.animateWalk(TortoiseAnimation.WALK, entityRenderState.walkAnimationPos, entityRenderState.walkAnimationSpeed, 8, 20);
//            AnimUtil.animateWalk(this, TortoiseAnimation.WALK, pLimbSwing, pLimbSwingAmount, 8, 20);
        }
        this.animate(entityRenderState.hideState, TortoiseAnimation.HIDE, entityRenderState.ageInTicks);
        this.animate(entityRenderState.hiddenState, TortoiseAnimation.HIDING, entityRenderState.ageInTicks);
        this.animate(entityRenderState.reveal_state, TortoiseAnimation.REVEAL, entityRenderState.ageInTicks);
    }
