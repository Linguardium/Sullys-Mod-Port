...
import static com.uraneptus.sullysmod.core.other.SMLocationUtil.lootTableKey;
        ...
        if (template.placeInWorld(level, new BlockPos(offsetPos.getX(), offsetPos.getY() - 2, offsetPos.getZ()), offsetPos, placeSettings, random, Block.UPDATE_ALL)) {
            if (variant.allowAmber()) {
                ...
                for (int i = 0; i < tries; i++) {
                    ...
                    config.amberBlobs().value().place(level, context.chunkGenerator(), random, amberPos);
                }
                return true;
            }
        }
                ...
                new RuleProcessor(
                        List.of(
                                new ProcessorRule(
                                        ...
                                        new AppendLoot(lootTableKey(lootTable))
                                )
                        )
                ),
